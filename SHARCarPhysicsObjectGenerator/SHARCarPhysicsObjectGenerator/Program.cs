...
try
{
    ...
    if (!noHistory)
        car.Chunks.Add(new HistoryChunk([$"Physics Objects Generated by SHARCarPhysicsObjectGenerator v{Assembly.GetExecutingAssembly().GetName().Version}.", $"SHARCarPhysicsObjectGenerator \"{string.Join("\" \"", args)}\"", $"Run at {DateTime.Now:R}"]));
